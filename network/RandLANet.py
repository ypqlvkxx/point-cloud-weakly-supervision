...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc0 = pt_utils.Conv1d(4, 8, kernel_size=1, bn=True)
        ...
        self.representation_blocks = nn.ModuleList()
        for j in range(self.config['num_layers']):
            if j < 3:
                d_in = d_out_rep + 2 * self.config['rep_d_out'][-j-2]
                d_out_rep = 2 * self.config['rep_d_out'][-j-2]
            else:
                d_in = 4 * self.config['rep_d_out'][-4]
                d_out_rep = 2 * self.config['rep_d_out'][-4]
            self.representation_blocks.append(pt_utils.Conv2d(d_in, d_out_rep, kernel_size=(1, 1), bn=True))
        

        self.fc1 = pt_utils.Conv2d(d_out, 64, kernel_size=(1, 1), bn=True)
        self.fc2 = pt_utils.Conv2d(64, 32, kernel_size=(1, 1), bn=True)
        self.dropout = nn.Dropout(0.5)
        self.fc3 = pt_utils.Conv2d(32, self.config['numclass'], kernel_size=(1, 1), bn=False, activation=None)

        #self.representation = pt_utils.Conv2d(32, 32, kernel_size=(1, 1), bn=False, activation=None)
    ...
    def forward(self, end_points):
        ...
        # ###########################Representation############################
        f_rep_list = []
        representation = self.decoder_0(f_encoder_list[-1])
        for j in range(self.config['num_layers']):
            f_interp_i = self.nearest_interpolation(representation, end_points['interp_idx'][-j - 1])
            f_rep_i = self.representation_blocks[j](torch.cat([f_encoder_list[-j - 2], f_interp_i], dim=1))

            representation = f_rep_i
            f_rep_list.append(f_rep_i)
        # ###########################Representation############################
        
        ###########################visulazation############################
        # f_vi_list = []
        # visul = self.decoder_0(f_encoder_list[-1])
        # for j in range(self.config['num_layers']):
        #     f_vi_i = self.nearest_interpolation(visul, end_points['interp_idx'][-j - 1])
        #     #f_rep_i = self.representation_blocks[j](torch.cat([f_encoder_list[-j - 2], f_interp_i], dim=1))
        #     visul = f_vi_i
        #     f_vi_list.append(f_vi_i)
        ###########################Representation############################
        ...
        end_points['rep'] = representation.squeeze(3)
        #end_points['f_decoder_i'] = f_vi_list[-1]
